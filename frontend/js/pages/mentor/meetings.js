...
export async function render(container) {
          ...
          <form id="sched-form">
            ...
            <div class="form-group" style="margin-bottom:14px;">
              <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                <label class="form-label" style="font-weight:600; margin:0;">Why / Agenda / Discussion Topics</label>
                <button type="button" class="btn-ai-sparkle" id="btn-ai-generate-agenda">
                  <i class="ph-bold ph-sparkle"></i> ✨ AI Generate Agenda
                </button>
              </div>
  ...
  container.querySelector('#btn-ai-generate-agenda')?.addEventListener('click', async () => {
    ...
    const studentName = studentId === 'ALL' ? 'Cohort Group (All Mentees)' : (selectedStudent?.name || 'Mentee');
    const dept = user?.department || 'Department';

    const aiBtn = container.querySelector('#btn-ai-generate-agenda');
    if (aiBtn) {
      aiBtn.disabled = true;
    ...
    try {
      ...
      const descEl = container.querySelector('#sched-desc');
      if (descEl) descEl.value = agenda;
      showToast('✨ Structured meeting agenda generated by AI!', 'success');
    } catch (e) {
      ...
      if (aiBtn) {
        ...
        aiBtn.innerHTML = '<i class="ph-bold ph-sparkle"></i> ✨ AI Generate Agenda';
      }
    }
  });
